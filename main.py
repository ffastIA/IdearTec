...
caminho = r"IdearTec1.pdf"

diretorio = 'VectorStore'
...
print(f"Carregando documentos do PDF: {caminho}")  # Informa o usuário sobre o carregamento do PDF.
try:  # Inicia um bloco try-except para lidar com possíveis erros durante o carregamento do arquivo.
    loader = PyPDFLoader(caminho)  # Instancia um PyPDFLoader, passando o caminho do arquivo PDF.
    ...
    print(
        f"Erro: O arquivo PDF não foi encontrado em '{caminho}'. Por favor, verifique o caminho.")  # Informa o erro ao usuário.
...
while True:  # Inicia um loop infinito que só será interrompido pela escolha do usuário.
    ...
    if escolha == '1':
        ...
        pergunta = input(
            "\nDigite sua pergunta: ")  # A variável 'pergunta' agora capta a entrada do usuário via terminal.
        ...
        # 2. Acesse a resposta principal
        resposta_llm = ""
        if 'result' in resposta_do_chat:
            resposta_llm = resposta_do_chat['result']
        else:
            print("A chave 'result' não foi encontrada na resposta.")

        print("--- Resposta do Consultor Utilizando IA ---")
        if resposta_llm:
            # Use textwrap.fill para formatar a resposta
            # Você pode ajustar o 'width' para a largura desejada,
            # por exemplo, 80, 100, etc.
            resposta_formatada = textwrap.fill(resposta_llm, width=120)
            print(resposta_formatada)
        else:
            print("Nenhuma resposta do LLM para formatar.")
